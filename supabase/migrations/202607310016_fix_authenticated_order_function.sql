-- Fixes an ambiguous `id` reference in migration 015's order function.
-- This changes no stored data.
...
declare
  ...
  selected_delivery_method public.delivery_methods;
  item_count integer;
...
begin
  ...
  select * into selected_delivery_method
  from public.delivery_methods as method
  where method.id = p_delivery_method_id and method.is_active = true;
  ...
  with requested as (
    ...
    select requested.product_id, requested.quantity, product.price
    from requested
    join public.products as product on product.id = requested.product_id and product.is_active = true
  )
  ...
  calculated_total := calculated_subtotal + selected_delivery_method.cost;
  ...
  insert into public.delivery_orders (
    ...
    selected_delivery_method.name, calculated_subtotal, selected_delivery_method.cost, calculated_total
  ) returning * into new_order;

  insert into public.delivery_order_items (order_id, product_id, product_name, quantity, unit_price)
  select new_order.id, product.id, product.name, (item->>'quantity')::integer, product.price
  from jsonb_array_elements(p_items) as item
  join public.products as product on product.id = item->>'product_id' and product.is_active = true;
